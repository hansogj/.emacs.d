...
(defvar mfinn-target-dir "/target/finn-mobile-3.0.0-SNAPSHOT")

(defun mfinn-hot-deploy-buffer-file ()
...
in an exploded war, re-deploy the file."
  ...
  (let* ((source (buffer-file-name))
         (target (s-with source
                   (s-replace "/src/main/webapp" mfinn-target-dir))))
...
(define-minor-mode mfinn-mode
  "Convenience utilities for working with mfinn"
  nil " mfinn" nil
  (if mfinn-mode
      (add-hook 'after-save-hook 'mfinn-hot-deploy-buffer-file nil t)
    (remove-hook 'after-save-hook 'mfinn-hot-deploy-buffer-file t)))


(provide 'mfinn-mode)
;;; mfinn-mode.el ends here
