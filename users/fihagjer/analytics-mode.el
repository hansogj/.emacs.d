...
;;(defvar analytics-target-dir "/finn/svn/trunk/licensee-finn/finn/overlays/no.finntech.commons-web-*")
(defvar analytics-target-dir "/finn/svn/trunk/licensee-finn/finn/target/exploded")
(defun analytics-hot-deploy-buffer-file ()
...
in an exploded war, re-deploy the file."
  ...
  (let* ((source (buffer-file-name))
         (target (s-with source
                   (s-replace "/finn/repo/common-web/trunk/analytics-js/src/main/webapp" analytics-target-dir))))
...
(define-minor-mode analytics-mode
  "Convenience utilities for working with Finn IAD"
  nil " IAD" nil
  (if analytics-mode
      (add-hook 'after-save-hook 'analytics-hot-deploy-buffer-file nil t)
    (remove-hook 'after-save-hook 'analytics-hot-deploy-buffer-file t)))

(eval-after-load "grep"
  '(progn (add-to-list 'grep-find-ignored-directories "tinymce")
          (add-to-list 'grep-find-ignored-directories "external")))

(provide 'analytics-mode)
;;; analytics-mode.el ends here
